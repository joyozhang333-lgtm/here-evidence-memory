...
The optional claim-verification corpus exposes full eligible user utterances to the calling process. Keep it ephemeral and server/local-only: never put it in the model prompt, logs, analytics, shared cards, or client responses. Its `complete` flag depends on the host's truthful `fullHistoryLoaded` assertion and cannot prove database completeness. It supplies material to a separate host-owned claim guard; it does not detect semantic contradictions, determine whether a user has changed, or authorize stronger psychological conclusions.
