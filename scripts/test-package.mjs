...
if (EVIDENCE_MEMORY_VERSION !== '0.2.0' || result.evidence.length !== 1 || !isSourceEligible(source, options) || !verifyEvidence(result.evidence[0], source, options) || !formatEvidenceContext(result).includes(source.id)) throw new Error('Packed ESM consumer failed');
`;
...
void [diagnostic, coverage, precision, verified, eligible, context, id];
`;
